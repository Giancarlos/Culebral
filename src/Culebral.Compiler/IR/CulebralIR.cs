...
// Async await — suspends via IAsyncStateMachine when await points exist, else synchronous fallback
public sealed record IrAwait(bool HasResult, int AwaitId, SourceSpan Span) : IrInstruction(Span);
