...
/// <summary>
/// Indentation-aware tokenizer for Skelvon source code.
...
/// </summary>
public sealed class SkelvonLexer
{
    ...
    public SkelvonLexer(string source, string filePath, DiagnosticBag diagnostics)
    {
        _source = source;
        _filePath = filePath;
    ...
    private void ProcessIndentation()
    {
        ...
        else if (indent < currentIndent)
        {
            ...
            {
                _diagnostics.Error("SKV0001", $"Inconsistent indentation: expected {_indentStack.Peek()} spaces, got {indent}",
                    SourceSpan.From(loc));
            }
        }
    }
    ...
    private Token ScanString()
    {
        ...
        while (_pos < _source.Length)
        {
            ...
            {
                _diagnostics.Error("SKV0002", "Unterminated string literal", new SourceSpan(start, CurrentLocation()));
                break;
            }
            else
            {
    ...
    private Token ScanFString()
    {
        ...
        while (_pos < _source.Length && Current != quote)
        {
            ...
            {
                _diagnostics.Error("SKV0002", "Unterminated f-string literal", new SourceSpan(start, CurrentLocation()));
                break;
            }
            else
            {
    ...
    private Token ScanNumber()
    {
        ...
        if (isFloat)
        {
            ...
            _diagnostics.Error("SKV0003", $"Invalid float literal: {text}", span);
        ...
        else
        {
            ...
            _diagnostics.Error("SKV0003", $"Invalid integer literal: {text}", span);
    ...
    private Token ScanOperatorOrDelimiter()
    {
        ...
        switch (ch)
        {
            ...
            case '!':
                ...
                _diagnostics.Error("SKV0004", "Unexpected character '!'", SourceSpan.From(start));
                return NextToken();
            ...
            default:
                _diagnostics.Error("SKV0004", $"Unexpected character '{ch}'", SourceSpan.From(start));
                return NextToken();
        }
    }
