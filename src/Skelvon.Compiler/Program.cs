...
public static class Program
{
    ...
    private static int HandleBuild(string[] args)
    {
        if (args.Length == 0)
        {
            ...
            Console.Error.WriteLine("Usage: skelvon build <file.skv> [--output <path>]");
            return 1;
        }
    ...
    private static int HandleRun(string[] args)
    {
        ...
        var inputPath = args[0];
        var tempDir = Path.Combine(Path.GetTempPath(), "skelvon_run");
    ...
    private static int HandleCheck(string[] args)
    {
        ...
        // Lex
        var lexer = new SkelvonLexer(source, inputPath, diagnostics);
    ...
    private static int HandleLex(string[] args)
    {
        ...
        var diagnostics = new DiagnosticBag();
        var lexer = new SkelvonLexer(source, args[0], diagnostics);
        var tokens = lexer.Tokenize();
    ...
    {
        Console.WriteLine("Skelvon Compiler");
        Console.WriteLine();
        Console.WriteLine("Usage: skelvon <command> [options]");
        Console.WriteLine();
        Console.WriteLine("Commands:");
        Console.WriteLine("  build <file.skv>     Compile a Skelvon source file to .NET assembly");
        Console.WriteLine("  run <file.skv>       Compile and run a Skelvon source file");
        Console.WriteLine("  check <file.skv>     Type-check without compiling");
        Console.WriteLine("  lex <file.skv>       Print lexer tokens (debug)");
        Console.WriteLine("  parse <file.skv>     Print parse tree (debug)");
        Console.WriteLine("  ir <file.skv>        Print SkelvonIR (debug)");
        Console.WriteLine();
    ...
    {
        Console.WriteLine("skelvon 0.1.0-alpha");
        return 0;
    }
