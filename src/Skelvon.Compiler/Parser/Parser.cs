...
namespace Skelvon.Compiler.Parser;

/// <summary>
/// Recursive descent parser for Skelvon.
/// Consumes tokens from the lexer and produces an AST.
/// Handles indentation-based blocks via INDENT/DEDENT tokens.
/// </summary>
public sealed class SkelvonParser
{
    private readonly List<Token> _tokens;
    private readonly DiagnosticBag _diagnostics;
    private int _pos;

    public SkelvonParser(List<Token> tokens, DiagnosticBag diagnostics)
    {
        _tokens = tokens;
    ...
    private Statement ParseAsyncDef()
    {
        ...
        {
            _diagnostics.Error("SKV1001", "Expected 'def' after 'async'", Current.Span);
    ...
    private PropertyDef ParsePropertyDef()
    {
        ...
        while (!IsAtEnd && Current.Kind != TokenKind.Dedent)
        {
            ...
            {
                _diagnostics.Error("SKV1002", "Expected 'get' or 'set' in property definition", Current.Span);
                Advance();
            }
    ...
    private Statement ParseDecoratedDef()
    {
        ...
        _diagnostics.Error("SKV1003", "Decorator must be followed by a function or class definition", Current.Span);
    ...
    private WhileStatement ParseWhileStatement()
    {
        ...
        Expect(TokenKind.Colon);
        var body = ParseBlock();
        return new WhileStatement(condition, body, new SourceSpan(start, body.Span.End));
    }
    ...
    private ForStatement ParseForStatement()
    {
        ...
        Expect(TokenKind.Colon);
        var body = ParseBlock();
        return new ForStatement(variable, iterable, body, new SourceSpan(start, body.Span.End));
    }
    ...
    private Pattern ParsePattern()
    {
        ...
        _diagnostics.Error("SKV1004", $"Unexpected token in pattern: {Current.Kind}", Current.Span);
        Advance();
        return new WildcardPattern(SourceSpan.From(start));
    }
    ...
    private Expression ParsePrimary()
    {
        ...
        switch (tok.Kind)
        {
            ...
            default:
                _diagnostics.Error("SKV1005", $"Unexpected token: {tok.Kind} '{tok.Lexeme}'", tok.Span);
                Advance();
                return new NoneLiteralExpr(tok.Span);
        }
    }
    ...
    private FStringExpr ParseFStringParts(string raw, SourceSpan span)
    {
        ...
        while (i < raw.Length)
        {
            if (raw[i] == '{')
            {
                ...
                var exprLexer = new SkelvonLexer(exprText, "<fstring>", _diagnostics);
                var exprTokens = exprLexer.Tokenize();
                var exprParser = new SkelvonParser(exprTokens, _diagnostics);
    ...
    private void ExpectNewlineAndIndent()
    {
        ...
        if (Current.Kind == TokenKind.Indent)
            ...
            _diagnostics.Error("SKV1006", "Expected indented block", Current.Span);
    }
    ...
    private Token Expect(TokenKind kind)
    {
        ...
        _diagnostics.Error("SKV1007", $"Expected {kind}, got {Current.Kind} '{Current.Lexeme}'", Current.Span);
        return Current;
    }
